...
const char* formRequest(string method, string url, string host, string port) {
    ...
    req.setHeader("Keep-Alive", to_string(KEEP_ALIVE));
...
int main(int argc, char *argv[]) {
	...
	//HTTP 1.1 added
	map<pair<string, string>, pair<int, double> > sock_map;
    ...
    for(int i = 1; i < argc; i ++) {
        ...
        pair<string, string> host_port_pair = make_pair(host, port);
        if(sock_map.find(host_port_pair) != sock_map.end()) {
        	pair<int, double> sock_time_pair = sock_map[host_port_pair];
        	gettimeofday(&cur_time , NULL);
        	double start_time = sock_time_pair.second;
        	sockfd = sock_time_pair.first;
        	if((cur_time.tv_sec - start_time) + 1e-6 * (cur_time.tv_usec - start_time) > KEEP_ALIVE){
        		close(sockfd);
        		create_new = true;
        	}
        	else sock_map[host_port_pair].second = cur_time.tv_sec;
        	int error_code;
        	socklen_t error_code_size = sizeof(error_code);
        	getsockopt(sockfd, SOL_SOCKET, SO_ERROR, &error_code, &error_code_size);
        	if(error_code != 0) create_new = true;
        }
        ...
        if(create_new) {
        	...
        	gettimeofday(&cur_time , NULL);
        	sock_map[host_port_pair] = make_pair(sockfd, cur_time.tv_sec);
        }
