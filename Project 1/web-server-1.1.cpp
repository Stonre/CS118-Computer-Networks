...
#define BUFSIZE 2048
#define TIMEOUT CLOCKS_PER_SEC*30
...
void childConnection(int clientSockfd) {
	...
	while (1) {
		if (double(clock() - tStart) > TIMEOUT) {
			close(clientSockfd);
			cout<<"Connection for clientSockfd "<<clientSockfd<<"time out and close connection"<<endl;
			return;
		}
		...
		if (!request.consume(requestStream) || request.getHeaderByKey("Host").size() == 0) {
			...
			if (!file.good()) {
				...
				response.setHeader("Keep-Alive", "timeout=" + std::to_string(TIMEOUT) + ", max=100");
			}
		}
